...
namespace PowerScope.Model
{
    ...
    public class SerialDataStream : IDataStream, IChannelConfigurable, IBufferResizable
    {
        ...
        // Sinc interpolation fields
        private const int InterpolationFactor = 5;
        ...
        /// <summary>
        /// Gets or sets whether sinc interpolation is enabled
        /// When enabled, incoming data is upsampled by 5x using sinc interpolation
        /// </summary>
        public bool SincInterpolationEnabled
        {
            get { return _sincInterpolationEnabled; }
            set
            {
                if (_sincInterpolationEnabled != value)
                {
                    _sincInterpolationEnabled = value;
                    OnPropertyChanged(nameof(SincInterpolationEnabled));

                    // If enabling, generate the sinc kernel
                    if (value)
                    {
                        GenerateSincKernel();
                    }
        ...
        private void GenerateSincKernel()
        {
            ...
            double cutoffFrequency = 0.75 / (2.0 * InterpolationFactor); // Nyquist frequency for upsampled signal
        ...
        /// <summary>
        /// Applies 5x sinc interpolation to input data
        /// Improved version that reduces boundary artifacts by using mirrored padding
        /// </summary>
        /// <param name="inputData">Input samples to interpolate</param>
        /// <returns>Interpolated samples (5x longer than input)</returns>
        private double[] SincInterpolation(double[] inputData)
        {
            if (inputData == null || inputData.Length == 0 || _sincKernel == null)
                return inputData;

            int inputLength = inputData.Length;
            int outputLength = inputLength * InterpolationFactor;
            double[] output = new double[outputLength];

            // Create extended input with mirrored boundaries to reduce edge effects
            int padLength = SincKernelHalfLength;
            double[] paddedInput = new double[inputLength + 2 * padLength];

            // Mirror boundary conditions - reflect samples at edges
            ...
            // Zero-pad and upsample the padded input
            int paddedOutputLength = paddedInput.Length * InterpolationFactor;
            double[] upsampled = new double[paddedOutputLength];

            for (int i = 0; i < paddedInput.Length; i++)
            {
                upsampled[i * InterpolationFactor] = paddedInput[i] * InterpolationFactor;
            }

            // Convolve with sinc kernel
            int kernelHalfLength = SincKernelHalfLength;
            int outputStartOffset = padLength * InterpolationFactor; // Offset to skip the padded region in output

            for (int i = 0; i < outputLength; i++)
            {
                double sum = 0.0;
                int upsampledIndex = i + outputStartOffset;

                for (int j = 0; j < _sincKernel.Length; j++)
                {
                    int sampleIndex = upsampledIndex - kernelHalfLength + j;
        ...
        public int BufferSize
        {
            ...
            set
            {
                ...
                lock (_channelConfigLock)
                {
                    ...
                    // For sinc interpolation, allocate 5x buffer size to accommodate upsampled data
                    int actualBufferSize = _sincInterpolationEnabled ? value * InterpolationFactor : value;
                    InitializeRingBuffers(actualBufferSize);
            ...
            for (int channel = 0; channel < channelsToProcess; channel++)
            {
                if (parsedData[channel] != null && parsedData[channel].Length > 0)
                {
                    ...
                    for (int sample = 0; sample < parsedData[channel].Length; sample++)
                    {
                        ...
                        // Safety check for invalid values that could cause interpolation artifacts
        ...
        /// When sinc interpolation is enabled, the reported sample rate reflects the interpolated rate for proper time base calculations
        /// </summary>
        private void UpdateSampleRateCalculation(int newSamples)
        {
            lock (_sampleRateCalculationLock)
            {
                ...
                if (elapsedSeconds >= 0.5)
                {
                    ...
                    // The instantaneous sample rate already includes interpolation factor since TotalSamples
                    // includes interpolated samples. This gives us the effective sample rate for time base calculations.
                    // For example: if hardware samples at 1000Hz and we do 5x interpolation,
                    // the effective sample rate becomes 5000Hz, which is what plotting and time base need.
